...
export class Aggregator extends Variable {
    constructor(props) {
        super(props);
        this.timer = props.timer;
        if (!this.timer) {
            throw Error("Aggregator needs timer");
        }
        let self = this;
        this.previousAgg = -1;
        this.lastChange = 0;
    ...
    aggregate() {
        if (this.baseValue !== this.currentValue) {
            this.setNewBaseValue(this.currentValue);
        }
...
export class AggregatorComponent extends VariableComponent {
    constructor(props) {
        super(props)
...
AggregatorComponent.defaultProps = {
    showChange: true
};